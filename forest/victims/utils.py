...
def print_and_save_stats(epoch, current_lr, train_loss, train_acc, predictions,
    ...
    write(f'Epoch: {epoch:<3}| lr: {current_lr:.4f} | '
        f'Training loss is {train_loss:7.4f}, Training accuracy: {train_acc:7.4%} | ', output)
    ...
    if predictions is not None:
        ...
        write(f'Validation  loss is {valid_loss:7.4f} | Validation accuracy: {valid_acc:7.4%}', output)
        write(f'Target val. acc  is {valid_acc_target:7.4%} | Source val accuracy: {valid_acc_source:7.4%}', output)
